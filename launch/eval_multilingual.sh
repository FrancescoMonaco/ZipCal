...
MAX_JOBS=3          # number of parallel jobs (GPUs 0-3 for eval_multilingual.sh)
...
DATASETS=(
    ...
    "xcopa_zh"
    "xquad_zh"
    "xnli_es xquad_es" #spanish only
    "xnli_zh xquad_zh" #chinese only
    "xnli_es xquad_es xnli_zh xquad_zh" #mixed
)
        ...
        for P_TYPE in "${PRUNING_TYPES[@]}"; do
            ...
            for COMP in "${COMPRESSION_TYPE[@]}"; do
                for NSAMPLES in "${NUM_SAMPLES[@]}"; do
                    ...
                    LOG="logs/eval_multilingual_t${TASK_ID}_s${SUBTASK_ID}_gpu${GPU_SLOT}.log"
                    launch_job "$GPU_SLOT" \
